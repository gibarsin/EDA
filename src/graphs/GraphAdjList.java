...
public abstract class GraphAdjList<V, E extends ArcGraph> {
    ...
    public Map<V, Double> Dijkstra(V origin) {
        ...
        do {
            min = null;
            minDist = 0.0;
            for (Node node : visited) { //Para todos los nodos a los cuales ya encontré su camino mínimo
                ...
                for (Arc e : node.adj) { //Para todos sus adyacentes
                    if (!e.neighbor.visited //Si no encontré un camino mínimo para este vecino
                            && (min == null || e.info.getValue() + distToNode < minDist)) { //min es solo para el primer caso o que los vecinos hasta ahora ya hayan sido visitados, la otra condición es para comparar las distancias minimas encontradas hasta ahora
                        min = e; //Para guardar en el mapa el nodo y marcar a este posible candidato como visitado en caso de que se haya encontrado el camino mínimo para éste.
